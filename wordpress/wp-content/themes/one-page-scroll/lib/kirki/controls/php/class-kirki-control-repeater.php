...
class Kirki_Control_Repeater extends Kirki_Control_Base {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->row_label = array(
			'type'  => 'text',
			'value' => esc_attr__( 'row', 'one-page-scroll' ),
			'field' => false,
		);
		...
		if ( empty( $this->button_label ) ) {
			...
			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'one-page-scroll' ), $this->row_label['value'] );
		}
					...
					case 'dropdown-pages':
						...
						$dropdown = wp_dropdown_pages(
							array(
								'name'              => '',
								'echo'              => 0,
								'show_option_none'  => esc_attr__( 'Select a Page', 'one-page-scroll' ),
								'option_none_value' => '0',
								'selected'          => '',
							)
						);
	...
	protected function render_content() {
		...
		<?php if ( isset( $this->choices['limit'] ) ) : ?>
			...
			<p class="limit"><?php printf( esc_attr__( 'Limit: %s rows', 'one-page-scroll' ), esc_html( $this->choices['limit'] ) ); ?></p>
		<?php endif; ?>
		...
		<script type="text/html" class="customize-control-repeater-content">
			...
			<li class="repeater-row minimized" data-row="{{{ index }}}">
				...
				<div class="repeater-row-content">
					<# _.each( data, function( field, i ) { #>
						...
						<div class="repeater-field repeater-field-{{{ field.type }}} repeater-field-{{ field.id }}">
							...
							<# } else if ( 'color' === field.type ) { #>
								...
								<input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'one-page-scroll' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
							...
							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
								...
								<figure class="kirki-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'one-page-scroll' ); ?>" >
									<# if ( field.default ) { #>
										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
										<img src="{{{ defaultImageURL }}}">
									<# } else { #>
										<?php esc_attr_e( 'No Image Selected', 'one-page-scroll' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_attr_e( 'Remove', 'one-page-scroll' ); ?></button>
									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'one-page-scroll' ); ?>" data-alt-label="<?php echo esc_attr_e( 'Change Image', 'one-page-scroll' ); ?>" >
										<# if ( field.default ) { #>
											<?php esc_attr_e( 'Change Image', 'one-page-scroll' ); ?>
										<# } else { #>
											<?php esc_attr_e( 'Add Image', 'one-page-scroll' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
							...
							<# } else if ( field.type === 'upload' ) { #>
								...
								<figure class="kirki-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'one-page-scroll' ); ?>" >
									<# if ( field.default ) { #>
									...
									<# } else { #>
										<?php esc_attr_e( 'No File Selected', 'one-page-scroll' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'one-page-scroll' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'one-page-scroll' ); ?>">
										<# if ( field.default ) { #>
											<?php esc_attr_e( 'Change File', 'one-page-scroll' ); ?>
										<# } else { #>
											<?php esc_attr_e( 'Add File', 'one-page-scroll' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
					...
					<# }); #>
					<button type="button" class="button-link repeater-row-remove"><?php esc_attr_e( 'Remove', 'one-page-scroll' ); ?></button>
				</div>
			</li>
		</script>
		<?php
