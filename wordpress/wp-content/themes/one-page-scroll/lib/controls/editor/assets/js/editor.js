...
var WPEditorWidget = {
    ...
    showEditor: function(contentId) {
        ...
        jQuery('body.wp-customizer #hoo-editor-container').fadeIn(100).animate({ 'bottom': '0' });

        this.currentContentId = contentId;
        this.wpFullOverlayOriginalZIndex = parseInt(overlay.css('zIndex'));
        overlay.css({ zIndex: 49000 });

        var editor = tinyMCE.EditorManager.get('hoo-page-editor');
        var content = jQuery("textarea[id='" + contentId + "']").val();

        if (typeof editor === 'object' && editor !== null) {
            editor.setContent(content);
        }
        jQuery('#hoo-page-editor').val(content);
    },
    ...
    hideEditor: function() {
        ...
        jQuery('body.wp-customizer #hoo-editor-container').animate({ 'bottom': '-650px' }).fadeOut();
    ...
    updateWidgetAndCloseEditor: function() {
        ...
        jQuery('#hoo-page-editor-tmce').trigger('click');
        var editor = tinyMCE.EditorManager.get('hoo-page-editor');
        var content = editor.getContent();

        if (typeof editor === 'undefined' || editor === null || editor.isHidden()) {
            content = jQuery('#hoo-page-editor').val();
        }
