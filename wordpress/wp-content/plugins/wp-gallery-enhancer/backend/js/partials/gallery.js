...
class Gallery {
	 ...
	 * @param {object} flkty
	 */
	observeChanges(elem, magicgrid, flkty) {
				...
				for(let mutation of mutationsList) {
					if ('attributes' == mutation.type) {
						...
						if (flkty) {
							...
							for(let rnode of mutation.removedNodes) {
	 ...
	 */
	isMason(elem) {
		const isMason = this.masonCls.filter(selector => elem.classList.contains(selector) );
	 ...
	 */
	isSlider(elem) {
		const isSlider = this.slidercls.filter(selector => elem.classList.contains(selector) );
